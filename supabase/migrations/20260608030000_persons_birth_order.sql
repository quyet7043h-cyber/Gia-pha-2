-- persons.birth_order — explicit sibling rank ("con thứ mấy").
--
-- Vietnamese genealogy convention is strict about anh/chị/em ranking;
-- legacy datasets often lack precise birth dates (some only know
-- "con thứ ba" by family memory). Without an explicit rank, the tree
-- and PersonDetail relationship lists fall back to birth_date order
-- which is wrong for twins / unknown-year cases.
--
-- Convention:
--   - 1 = con cả (oldest), 2 = con thứ hai, …
--   - NULL means "unknown / not set" → consumers fall back to
--     birth_date asc, then full_name.
--   - No uniqueness constraint per family — allows fixing data in
--     pieces (user might know ranks 1,2,5 but not 3,4 yet).
--
-- Also updates `get_inlaw_peer_relatives` to honour the new rank
-- when ordering peer-side children (matches local-side ordering).

alter table public.persons
  add column if not exists birth_order int
    check (birth_order is null or birth_order > 0);

-- Cheap partial index for the common per-family sibling query.
create index if not exists persons_birth_family_order_idx
  on public.persons (birth_family_id, birth_order, birth_date)
  where deleted_at is null and birth_family_id is not null;

-- Refresh get_inlaw_peer_relatives so cross-clan family card sorts
-- children the same way as local family card.
