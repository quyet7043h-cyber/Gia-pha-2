-- Fix get_platform_db_stats — earlier migration used the non-existent
-- function `row_to_jsonb(record)` at the failed-notifications aggregate.
-- CREATE FUNCTION succeeded (Postgres validates the body lazily) but
-- calling the function on prod throws:
--   function row_to_jsonb(record) does not exist
--
-- Replace with `to_jsonb(record)` — the canonical row → jsonb cast
-- since 9.5. Rest of the function body is byte-for-byte identical to
-- 20260607123556_failed_notifications.sql.
...
begin
  ...
  -- Recent 10 failed notifications — `to_jsonb(r)` (NOT row_to_jsonb).
  select coalesce(
